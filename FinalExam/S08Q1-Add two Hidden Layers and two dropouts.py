...
# Class: Deep Learning
# Assignment: Final Exam, Chapter: 08, Q1, Book: "Python Machine Learning By Example"
import pandas as pd
...
def train_test_model(hparams, logdir):
    model = Sequential([
        ...
        Dense(units=hparams[HP_HIDDEN3], activation='relu'),
        Dense(units=1)
        ])
...
def run(hparams, logdir):
    with tf.summary.create_file_writer(logdir).as_default():
        hp.hparams_config(
            hparams=[HP_HIDDEN1, HP_HIDDEN2, HP_HIDDEN3, HP_DROPOUT1, HP_DROPOUT2, HP_EPOCHS, HP_LEARNING_RATE],
...
HP_HIDDEN3 = hp.HParam('3rd hidden layer', hp.Discrete([64, 32, 16]))
                ...
                for dropout2 in (HP_DROPOUT2.domain.min_value, HP_DROPOUT2.domain.max_value):
                    for epochs in HP_EPOCHS.domain.values:
                        for learning_rate in tf.linspace(HP_LEARNING_RATE.domain.min_value, HP_LEARNING_RATE.domain.max_value, 5):
                            ...
                            run(hparams, 'logs8N5/hparam_tuning/' + run_name)
                            session_num += 1
